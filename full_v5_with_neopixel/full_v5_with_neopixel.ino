...
#define BUTTON_PIN 39 //Menu switch button
#define REC_BUTTON_PIN 35 //Button to stop GoPro recording
...
int recordingButton = 0;     // button state for recording
int recButtonState = 0;      // current state of recording button
int lastRecButtonState = 0;  // previous state of recording button
int recButtonPushCounter = 0; // counter to determine button has been pushed
int flag = 0;                // semaphore used to turn off recording. Happens once.
...
void setup() {
  ...
  pinMode(REC_BUTTON_PIN, INPUT); //initialize pushbutton pin for recording as an input
...
//-----------------------------------------------------------------------
// Push button for GoPro stop recording
//-----------------------------------------------------------------------
    recButtonState = digitalRead(REC_BUTTON_PIN);
    // compare the buttonState to its previous state
  if (recButtonState != lastRecButtonState) {
    // if the state has changed, increment the counter
    if (recButtonState == HIGH) {
      // if the current state is HIGH then the button went from off to on:
      recButtonPushCounter++;
      Serial.println("on");
      Serial.print("number of button pushes: ");
      Serial.println(recButtonPushCounter);
